...
import { LogOut, Plus, Trash2, User, Briefcase, BookOpen, Layers, Mail, Calendar } from 'lucide-react';
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans flex flex-col md:flex-row">
      {/* SIDEBAR */}
      <aside className="w-full md:w-64 border-r border-zinc-800 p-6 flex flex-col bg-black z-50 fixed md:relative h-auto md:h-screen">
      ...
      {/* MAIN CONTENT */}
      <main className="flex-1 p-6 md:p-10 overflow-y-auto h-screen ml-0 md:ml-0 pt-24 md:pt-10">
                    ...
                    {projects.map(p => (
                        <div key={p._id} className="flex items-center justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
                            <div className="flex items-center gap-4">
                                {p.image && <img src={p.image.startsWith('http') ? p.image : `https://avi-portfolio-ssz8.onrender.com/${p.image}`} className="w-12 h-12 object-cover rounded" />}
        ...
        {activeTab === 'messages' && (
            <div className="max-w-4xl mx-auto">
                ...
                {messages.length === 0 ? <div className="text-center py-20 text-zinc-500">No messages yet.</div> : (
                    <div className="space-y-4">
                        {messages.map(msg => (
                            <div key={msg._id} className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800 relative group hover:border-[#00de51]/50">
                                <div className="flex justify-between items-start mb-4">
                                    <div><h3 className="font-bold text-lg">{msg.name}</h3><a href={`mailto:${msg.email}`} className="text-[#00de51] text-sm hover:underline">{msg.email}</a></div>
                                    <div className="text-right text-xs text-zinc-500">
                                        <p className="flex items-center gap-1 justify-end"><Calendar size={12}/> {new Date(msg.date).toLocaleDateString()}</p>
                                        <p>{new Date(msg.date).toLocaleTimeString()}</p>
                                    </div>
                                </div>
                                <div className="bg-black/50 p-4 rounded-lg border border-zinc-800/50 text-zinc-300 text-sm leading-relaxed">{msg.message}</div>
                                <button onClick={() => handleDeleteMessage(msg._id)} className="absolute top-6 right-6 text-zinc-600 hover:text-red-500"><Trash2 size={18} /></button>
                            </div>
                        ))}
                    </div>
                )}
        ...
        {activeTab === 'profile' && (
            <div className="max-w-2xl mx-auto">
                ...
                <form onSubmit={handleProfileUpdate} className="bg-zinc-900 p-8 rounded-2xl border border-zinc-800 space-y-6">
                    <input className="w-full bg-black p-3 rounded border border-zinc-700 text-white" placeholder="Full Name" value={profile.name} onChange={e => setProfile({...profile, name: e.target.value})} />
                    <input className="w-full bg-black p-3 rounded border border-zinc-700 text-white" placeholder="Roles" value={profile.roles} onChange={e => setProfile({...profile, roles: e.target.value})} />
                    <textarea className="w-full bg-black p-3 rounded border border-zinc-700 text-white" rows="4" placeholder="Bio" value={profile.bio} onChange={e => setProfile({...profile, bio: e.target.value})} />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <input className="bg-black p-3 rounded border border-zinc-700 col-span-2" placeholder="Email" value={profile.email} onChange={e => setProfile({...profile, email: e.target.value})} />
                    </div>
        ...
        {/* === TAB: EXP & EDU & TECH (Simplified for brevity but functional) === */}
        {activeTab === 'resume_data' && (
            <div className="max-w-3xl mx-auto space-y-10">
                {/* Same Exp/Edu Logic as provided in your prompt */}
                <div>
                    <div className="flex justify-between items-center mb-4"><h2 className="text-2xl font-bold">Experience</h2><button onClick={() => addItem('experience', { role: '', company: '', year: '', desc: '' })} className="bg-zinc-800 p-2 rounded hover:bg-zinc-700"><Plus size={20}/></button></div>
                    {profile.experience.map((exp, i) => (
                        <div key={i} className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 mb-4 relative group">
                            ...
                            <input className="bg-black p-2 rounded border border-zinc-700 w-full mb-2" placeholder="Role" value={exp.role} onChange={e => updateItem('experience', i, 'role', e.target.value)} />
                            <input className="bg-black p-2 rounded border border-zinc-700 w-full mb-2" placeholder="Company" value={exp.company} onChange={e => updateItem('experience', i, 'company', e.target.value)} />
                            <input className="bg-black p-2 rounded border border-zinc-700 w-full mb-2" placeholder="Year" value={exp.year} onChange={e => updateItem('experience', i, 'year', e.target.value)} />
                ...
                <div>
                    <div className="flex justify-between items-center mb-4"><h2 className="text-2xl font-bold">Education</h2><button onClick={() => addItem('education', { degree: '', school: '', year: '' })} className="bg-zinc-800 p-2 rounded hover:bg-zinc-700"><Plus size={20}/></button></div>
                    {profile.education.map((edu, i) => (
                        <div key={i} className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 mb-4 relative group">
                            ...
                            <input className="bg-black p-2 rounded border border-zinc-700 w-full mb-2" placeholder="Degree" value={edu.degree} onChange={e => updateItem('education', i, 'degree', e.target.value)} />
                            <input className="bg-black p-2 rounded border border-zinc-700 w-full mb-2" placeholder="School" value={edu.school} onChange={e => updateItem('education', i, 'school', e.target.value)} />
                            <input className="bg-black p-2 rounded border border-zinc-700 w-full" placeholder="Year" value={edu.year} onChange={e => updateItem('education', i, 'year', e.target.value)} />
                        </div>
                    ))}
                </div>
        ...
        {activeTab === 'tech' && (
            <div className="max-w-3xl mx-auto">
                 <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold">Tech Stack</h2><button onClick={() => addItem('techStack', { name: '', level: '50', color: '#ffffff' })} className="bg-zinc-800 p-2 rounded hover:bg-zinc-700"><Plus size={20}/></button></div>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
